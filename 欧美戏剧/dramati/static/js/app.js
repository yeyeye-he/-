(() => {
  ...
  function getSelectedAudience() {
    ...
    return r ? r.value : "fan_rare";
  }

  function syncAudienceCounts() {
    const list = meta.audiences || [];
    list.forEach((a) => {
      const el = document.querySelector(`[data-audience-count="${a.id}"]`);
      if (el) el.textContent = String(a.count);
    });
  }

  function syncQCountLine() {
    const id = getSelectedAudience();
    const row = (meta.audiences || []).find((x) => x.id === id);
    if (row) $("q-count").textContent = String(row.count);
  }

  async function loadMeta() {
    const r = await fetch("/api/meta");
    meta = await r.json();
    $("app-subtitle").textContent = meta.subtitle || "";
    $("app-disclaimer").textContent = meta.disclaimer || "";
    $("app-title").textContent = meta.title || "DRAMA-TI";
    syncAudienceCounts();
    syncQCountLine();
  }

  async function loadQuestions(aud) {
    const r = await fetch(`/api/questions?audience=${encodeURIComponent(aud)}`);
  ...
  document.querySelectorAll('input[name="audience"]').forEach((inp) => {
  ...
  $("btn-start").addEventListener("click", async () => {
    ...
    const row = (meta.audiences || []).find((x) => x.id === audience);
    audienceLabel = row ? row.title : "";
    $("quiz-profile").textContent = row
      ? `本卷：${row.title} · ${row.count} 题`
      : "";
    await loadQuestions(audience);
    show(elQuiz);
    renderQuestion();
  });
